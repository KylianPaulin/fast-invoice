...
interface InvoiceOptions {
  ...
  data: {
    invoice: {
      ...
      customer: Array<{ label: string; value: string | Array<string> }>;
      seller: Array<{ label: string; value: string | Array<string> }>;
      details: {
...
export default class FastInvoice {
  ...
  private storage: {
    ...
    seller: { height: number };
    fonts: { fallback: { loaded: boolean } };
  ...
  constructor(options: Partial<InvoiceOptions>) {
    ...
    if (
      this.options.style.fonts?.fallback?.enabled &&
      this.options.style.fonts?.fallback?.path
    ) {
      if (fs.existsSync(this.options.style.fonts.fallback.path)) {
        this.storage.fonts.fallback.loaded = true;
      } else {
        console.warn(
          `Fallback font not found at ${this.options.style.fonts.fallback.path}`
        );
      }
    }
  }
  ...
  loadCustomFonts() {
    ...
    if (this.options.style.fonts?.normal?.path && this.storage.document) {
      this.storage.document.registerFont(
  ...
  getFontOrFallback(type: "normal" | "bold", value: string | number): string {
    ...
    if (/[\u20A0-\u20CF]/.test(value.toString())) {
      if (
        this.storage.fonts.fallback.loaded === false &&
    ...
    if (this.options.style.fonts?.fallback?.enabled === false) {
      return (
        this.options.style.fonts[type].name ??
        this.options.style.fonts?.normal?.name ??
        "Helvetica"
      );
    }
   ...
   * @return void
   */
  valueOrTransliterate(value: string) {
    let _fallbackRange = this.options.style.fonts?.fallback?.range;
    ...
    if (this.storage.document) {
      this.storage.document
        .rect(
          0,
          0,
          this.storage.document.page.width,
          this.options.style.header?.height ?? 150
        )
   ...
   * @return void
   */
  generateTableRow(type: "header" | "row", columns: any[]) {
    ...
    let _maxWidth =
      ((this.options.style.header?.textPosition ?? 330) -
        _start -
    ...
    columns.forEach((column, index) => {
      ...
      if (index === 0) {
        ...
        this.setCursor(
          "x",
          this.options.style.table?.beforeLast?.position ?? 0
        );
      } else {
      ...
      this.setText(_value, {
        colorCode: _colorCode as "primary" | "secondary",
        maxWidth: _maxWidth,
        fontWeight: _fontWeight as "normal" | "bold",
        skipDown: true,
      });
  ...
  generateLegal() {
    ...
    (this.options.data.invoice.legal || []).forEach((legal) => {
      ...
      this.setText(legal.value, {
        fontWeight: legal.weight as "normal" | "bold",
        colorCode: (legal.color as "primary" | "secondary") || "primary",
  ...
  setText(text: string, options: TextOptions = {}) {
    ...
    if (!_color && this.storage.document) {
      if (_colorCode === "primary") {
        this.storage.document.fillColor(
          this.options.style.text?.primaryColor ?? ""
        );
      } else {
        this.storage.document.fillColor(
          this.options.style.text?.secondaryColor ?? ""
        );
      }
    }
  ...
  async generate(
    ...
    if (
      ...
      (output as { type: string })?.type === "file"
    ) {
      let _path = "";
      ...
      if (typeof output === "string") {
        ...
        _path = (output as { path: string }).path;
      }
