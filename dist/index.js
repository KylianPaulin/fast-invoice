...
module.exports = class FastInvoice {
   ...
   */
  generateHeader() {
    this.document.rect(0, 0, this.document.page.width, this.options.style.header.height).fill(this.options.style.header.backgroundColor);
  ...
  generateTableRow(type, columns) {
    ...
    let _maxWidth = (this.options.style.header.textPosition - _start - this.options.style.document.marginRight) / (columns.length - 2);
  ...
  generateLine() {
    ...
    this.document.strokeColor("#F0F0F0").lineWidth(1).moveTo(this.options.style.document.marginRight, this.storage.cursor.y).lineTo(
